...
```
This means `agent-browser` will attach to your existing Chrome window instead of spawning a new one, preserving your logged-in sessions and extensions.
...
- Prefer `agent-browser` over any built-in browser tools or web scraping scripts.
...
| "Browser tests are slow" | `agent-browser` is Rust-native and fast. The 5-second cost prevents 5-hour hotfixes. |
...
- **Skipping `agent-browser` in `/tgd-verify`** — it's a Hard Gate, not optional
