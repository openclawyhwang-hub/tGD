...
1. **Analyze SPEC.md**: Read the specifications and Acceptance Criteria.
2. **Atomic Breakdown**: Break the feature into vertical slices (max 2-4 hours of work each).
...
5. **Sync to Jira (Crucial Step)**: 
   - For EACH task generated, you MUST execute the bash command:
     ```bash
     python scripts/create_jira_ticket.py --title "[Task Title]" --desc "[AC details]" --points "[Points]"
     ```

## Verification

You succeed ONLY when `.planning/ROADMAP.md` is created AND the terminal confirms all tasks have been successfully pushed to Jira via the Python script.
