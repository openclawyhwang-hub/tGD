...
2. **Verify**: Ensure `tGD/plan/` exists (created by `/tgd-define`). If missing, halt and ask user.
3. **Lock**: All planning artifacts go into `tGD/plan/`.
...
**Do NOT write code during planning.** Write a plan document at `tGD/plan/TASKS.md` covering: dependency graph, ordered task list with acceptance criteria, verification checkpoints, and risks with mitigations.

**TASKS.md template (save to `tGD/plan/TASKS.md`):**
...
- [ ] If UI feature: `tGD/plan/DESIGN.md` exists (created in Define phase)
