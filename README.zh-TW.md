...
跑完 tGD lifecycle 後，你的 workspace 會長這樣：

```
workspace/
├── my-project/                         # Code repo
│   ├── .codegraph → tGD/.codegraph     # CodeGraph CLI 用的 symlink
│   ├── tGD/
│   │   ├── .codegraph/                 # 符號索引（自動產生）
│   │   └── .understand-anything/       # 知識圖譜（自動產生）
│   ├── src/                            # 實作程式碼
│   └── tests/                          # 測試檔案
│
├── my-project-frontend/                # Code repo（可選）
│   ├── .codegraph → tGD/.codegraph
│   ├── tGD/
│   └── src/
│
└── my-project-tGD/                     # ← $TGD_DIR（同層級，不是裡面）
    ├── CONTEXT.md                      # 產品級 context + repo inventory
    ├── CHANGELOG.md                    # 統一版本記錄
    ├── .scans/                         # 集中存放掃描資料
    │   ├── my-project/
    │   │   ├── .codegraph/
    ...
    │       └── .understand-anything/
    │
    └── <feature-name>/                 # 每個功能一個資料夾
        ├── PRD.md                      # What & Why（10 個章節）
        ├── SPEC.md                     # How（標記 repo）
        ├── DESIGN.md                   # UI 設計（如適用）
        ├── prototype/                  # HTML mockups（如有 UI）
        │   ├── variant-a.html
        │   └── variant-b.html
        ├── TASKS.md                    # BDD 任務（標記 repo）
        ├── REVIEW.md                   # 程式碼審查 + 簡化報告
        └── decisions/                  # ADR（任何階段）
            └── ADR-001.md
```
...
當專案跨多個 repo 時，SPEC.md 和 TASKS.md 會用 repo 名稱標記：
...
CodeGraph 和 Understand-Anything 的輸出集中在 `$TGD_DIR/.scans/<repo>/`。每個 code repo 用 symlink 串接，工具不需要知道集中存放的位置：
...
這樣 code repo 保持乾淨 — 根目錄只有 `tGD/`（symlink 資料夾）和 `src/`。

#### 重要慣例

| 慣例 | 說明 |
|------|------|
| `$TGD_DIR` | 指向 tGD 資料夾的環境變數 |
| Feature branch | tGD repo 和 code repo 都建 `feature/<name>` |
| Branch 命名 | 所有 repo 用同一個名稱，方便對照 |
| CONTEXT.md | 一個產品一份，包含 repo inventory 表格 |
| CHANGELOG.md | 一個產品一份，迭代式版本記錄 |
| decisions/ | 任何階段都能產生 ADR，不綁 Define |
