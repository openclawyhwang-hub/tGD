...
├── tGD/
│   ├── map/                          ← /tgd-map 輸出
│   │   ├── CONTEXT.md                ← 專案總覽（主專案 + 額外 repos）
│   │   ├── .codegraph/               ← 符號索引（CodeGraph）
│   │   └── .understand-anything/     ← 知識圖譜（必選）
│   │
│   ├── define/                       ← /tgd-define 輸出（每個功能）
│   │   └── <feature-name>/           ← 使用者從 3 個選項中選擇（如 jwt-auth）
│   │       ├── PRD.md                ← 產品需求
│   │       ├── SPEC.md               ← 技術規格
│   │       ├── DESIGN.md             ← UI 設計（如適用）
│   │       └── prototype/            ← HTML mockups（如有 UI 功能）
│   │           ├── variant-a.html
│   │           └── variant-b.html
│   │
│   └── plan/                         ← /tgd-plan 輸出（每個功能）
│       └── <feature-name>/
│           └── TASKS.md              ← 任務拆解（讀取 CONTEXT + PRD + SPEC）
```

**說明：**
- `/tgd-develop` 在 git worktree 中工作（不會產生 tGD/ 輸出）
- `/tgd-verify`、`/tgd-review`、`/tgd-ship` 產生驗證結果，而非持久化檔案
