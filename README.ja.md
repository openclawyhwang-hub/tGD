...
├── tGD/
│   ├── map/                          ← /tgd-map の出力
│   │   ├── CONTEXT.md                ← プロジェクトコンテキスト（メイン + 追加リポジトリ）
│   │   ├── .codegraph/               ← シンボルインデックス（CodeGraph）
│   │   └── .understand-anything/     ← ナレッジグラフ（必須）
│   │
│   ├── define/                       ← /tgd-define の出力（機能ごと）
│   │   └── <feature-name>/           ← ユーザーが3つのオプションから選択（例：jwt-auth）
│   │       ├── PRD.md                ← 製品要件
│   │       ├── SPEC.md               ← 技術仕様
│   │       ├── DESIGN.md             ← UIデザイン（該当する場合）
│   │       └── prototype/            ← HTMLモックアップ（UI機能の場合）
│   │           ├── variant-a.html
│   │           └── variant-b.html
│   │
│   └── plan/                         ← /tgd-plan の出力（機能ごと）
│       └── <feature-name>/
│           └── TASKS.md              ← タスク分解（CONTEXT + PRD + SPEC を読み取る）
```

**注釈：**
- `/tgd-develop` は git worktree で動作（tGD/ 出力なし）
- `/tgd-verify`、`/tgd-review`、`/tgd-ship` は検証結果を生成し、永続ファイルは生成しない
