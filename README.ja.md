...
tGDライフサイクルを実行した後のワークスペース：

```
workspace/
├── my-project/                         # コードリポジトリ
│   ├── .codegraph → tGD/.codegraph     # CodeGraph CLI用シンボリックリンク
│   ├── tGD/
│   │   ├── .codegraph/                 # シンボルインデックス（自動生成）
│   │   └── .understand-anything/       # ナレッジグラフ（自動生成）
│   ├── src/                            # 実装コード
│   └── tests/                          # テストファイル
│
...
└── my-project-tGD/                     # ← $TGD_DIR（兄弟ディレクトリ）
    ...
    │       └── .understand-anything/
    │
    └── <feature-name>/                 # フィーチャー単位
        ├── PRD.md                      # What & Why（10セクション）
        ├── SPEC.md                     # How（リポジトリ別タグ）
        ├── DESIGN.md                   # UIデザイン（該当する場合）
        ├── prototype/                  # HTMLモックアップ（UIの場合）
        │   ├── variant-a.html
        │   └── variant-b.html
        ├── TASKS.md                    # BDDタスク（リポジトリ別タグ）
        ├── REVIEW.md                   # コードレビュー + 簡素化レポート
...
プロジェクトが複数のリポジトリにまたがる場合、SPEC.mdとTASKS.mdはリポジトリ名でタグ付けします：
...
CodeGraphとUnderstand-Anythingの出力は`$TGD_DIR/.scans/<repo>/`に集約されます。各コードリポジトリにはシンボリックリンクが設定され、ツールは集中ストレージを意識せずに動作します：
...
これによりコードリポジトリはクリーンに保たれます — ルートには`tGD/`（シンボリックリンクフォルダ）と`src/`のみ。
...
| ブランチ命名 | すべてのリポジトリで同じ名前を使用（トレーサビリティ向上） |
| CONTEXT.md | 製品ごとに1つ、リポジトリインベントリテーブルを含む |
| CHANGELOG.md | 製品ごとに1つ、反復的なバージョンログ |
| decisions/ | 任意のフェーズでADRを作成可能（Defineに限定されない） |
