...
エージェント：全テスト実行中...
→ 47件成功、1件失敗
→ test_login_empty_password：エラー予定が成功
...
8つのステージでアイデアから本番環境まで。各ステージが次のゲートを守ります。
...
tGDのテストは単一フェーズではなく、3段階の漸進的規律です：
...
| Agent | 役割 | 視点 |
|-------|------|------|
| [code-reviewer](agents/code-reviewer.md) | シニアスタッフエンジニア | 「スタッフエンジニアが承認するか？」 |
...
| performance-optimization | プロファイリング & アンチパターン |
</details>
